...
Фон снимается один раз при открытии окна и дальше не обновляется: размытие,
которое делает сама Windows, принадлежит системе, и наложить на него шейдер
нельзя — до этих пикселей приложение не дотягивается. Практически это значит,
что если под окном идёт видео, стекло покажет кадр на момент открытия.
